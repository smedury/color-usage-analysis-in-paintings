...
class PaintingsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = [
            'https://www.fridakahlo.org/frida-kahlo-paintings.jsp',
        ]
        for url in urls:
    ...
    def parse(self, response):
        ...
        for t in paintings:
            if len(t.css('a').xpath('@href')) == 1:
                ...
                paintingUrl = 'https://www.fridakahlo.org{}'.format(t.css('a').xpath('@href').extract()[0])
                #   Extact the image and save a file with the data inside
                yield scrapy.Request(url=paintingUrl, callback=self.extractData)

    def extractData(self, response):
        print(response)
        image = 'https://www.fridakahlo.org' + response.css('table img').xpath('@src').extract()[0]
        if image:
            #   Get the title and the year
            title_full = response.css('h1::text').extract()[0].strip().lower()
            if title_full:
                title_full = title_full.split(', ')
                title = title_full[0].strip()
                if(len(title_full)>1):
                    year = title_full[1].replace('by frida kahlo', '').strip()
                    year = re.sub('\D','',year)
