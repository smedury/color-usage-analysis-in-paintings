...
class PaintingsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = [
            'https://www.mark-rothko.org/mark-rothko-paintings.jsp',
        ]
        for url in urls:
    ...
    def parse(self, response):
        ...
        for t in paintings:
            if len(t.css('a').xpath('@href')) == 1:
                ...
                paintingUrl = 'https://www.mark-rothko.org{}'.format(t.css('a').xpath('@href').extract()[0])
                #   Extact the image and save a file with the data inside
                yield scrapy.Request(url=paintingUrl, callback=self.extractData)

    def extractData(self, response):
        print(response)
        image = 'https://www.mark-rothko.org' + response.css('table img').xpath('@src').extract()[0]
        #   Get the title and the year
        if image and len(response.css('h2 ::text').extract())>=1:
            title_full = response.css('h2 ::text').extract()[0].strip().lower()
            if title_full:
                title_full = title_full.split(', ')
                title = title_full[0].strip()
                if(len(title_full)>1):
                    year = title_full[1].replace('by mark rothko', '').strip()
                    year = re.sub('\D','',year)
